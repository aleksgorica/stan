...
TEST(sample_defaults, adaptation_engaged) {
  ...
  EXPECT_NO_THROW(
      adaptation_engaged::validate(adaptation_engaged::default_value()));
...
TEST(sample_defaults, stepsize_jitter) {
  ...
  EXPECT_THROW(stepsize_jitter::validate(-0.001), std::invalid_argument);
  EXPECT_THROW(stepsize_jitter::validate(1.0001), std::invalid_argument);
